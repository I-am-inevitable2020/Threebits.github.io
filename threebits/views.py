...
def home(request):
    ...
    return render(request, 'threebits/home.html',{'albums':albums})
...
def profile_detail(request, username):
    ...
    return render(request, 'threebits/artist_detail.html', {'albums':albums, 'username':username})
    ...
    if user == request.user:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect('threebits:profile_detail', username=request.user)
        else:
            form = NewAlbum()
        return render(request, 'threebits/add_new_album.html', {'form':form})
    else:
        return redirect('threebits:profile_detail', username=user)
...
def album_detail(request,username, album):
    ...
    return render(request, 'threebits/album_detail.html', {'songs':songs, 'album':album, 'username':username
    })
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            return redirect('threebits:home')
        else:
            message = 'Looks like a username with that email or password already exists'
            return render(request, 'threebits/signup.html', {'form':form,'message':message})
    else:
        form = SignUpForm()
        return render(request, 'threebits/signup.html', {'form':form})
...
def delete_album(request, username, album):
    ...
    if request.user == user:
        ...
        return redirect('threebits:profile_detail', username=username)
    else:
        return redirect('threebits:profile_detail', username=username)
    ...
    if request.user == user:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect('threebits:album_detail', username=username, album=album)

        else:
            form = NewSong()
            return render(request, 'threebits/add_new_song.html', {'form':form})
    else:
        return redirect('threebits:album_detail', username=username, album=album)
